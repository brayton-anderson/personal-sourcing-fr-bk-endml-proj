...
import requests
import pandas as pd
from website.searchgoogleapidata.settings import *
...
class Searchgoogle:
    ...
    def search(query):
        ...
        stored_results = DBStorage.query_results(query)
        ...
        results = Searchgoogle.search_api(query)
        html = Searchgoogle.scrape_page(results["link"])
        results["html"] = html
        results = results[results["html"].str.len() > 0].copy()
        results["query"] = query
        results["created"] = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
        results = results[columns]
        results.apply(lambda x: DBStorage.insert_row(x), axis=1)
